...
export default function IdeaBank({ ideas }: Props) {
  ...
  const themes = useMemo(() => {
    return ["All", ...Array.from(new Set(ideas.map((idea) => idea.theme).filter(Boolean)))] as string[];
  }, [ideas]);

  const statuses = useMemo(() => {
    return ["All", ...Array.from(new Set(ideas.map((idea) => idea.status).filter(Boolean)))] as string[];
  }, [ideas]);
        ...
        <tbody>
          {filteredIdeas.map((idea) => (
            <tr key={idea.id} className="border-t hover:bg-gray-50">
              ...
              <td className="p-4">{idea.theme || "-"}</td>
              <td className="p-4">{idea.language || "-"}</td>
              <td className="p-4">
                <span className="px-3 py-1 rounded-full bg-gray-100">
                  {idea.status || "Idea"}
                </span>
              </td>
              <td className="p-4 font-semibold">{idea.score || 0}</td>
              <td className="p-4">
